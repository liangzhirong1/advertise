...
/** 递归获取所有 adUnit key */
function getAllAdUnitKeys(nodes: OrganizationTreeNode[]): string[] {
  const keys: string[] = [];
...
/** 根据选中的 adUnit key 过滤广告计划 */
function filterAdPlansByAdUnits(
  plans: AdPlan[],
  selectedKeys: string[]
): AdPlan[] {
  if (selectedKeys.length === 0) return plans;
  return plans.filter((plan) => selectedKeys.includes(plan.adUnitId || ''));
}
...
const CampaignOperations: React.FC = () => {
  ...
  const [editingOcpmKey, setEditingOcpmKey] = useState<string>('');
  const [editOcpmValue, setEditOcpmValue] = useState<number>(0);
    ...
    const filterAndTransform = (
      ...
      return nodes
        .map((node) => {
          ...
          // 广告组/项目/单元节点显示状态
          if ((node.type === 'campaign' || node.type === 'project' || node.type === 'adUnit') && node.status) {
  ...
  const filteredAdPlans = useMemo(() => {
    let plans = filterAdPlansByAdUnits(mockAdPlans, checkedKeys as string[]);
  ...
  const onCheck = useCallback(
    (checked: React.Key[] | { checked: React.Key[]; halfChecked: React.Key[] }) => {
      // 只保留 adUnit 级别的 key
      const allAdUnitKeys = getAllAdUnitKeys(mockOrgTree);
      const checkedArray = Array.isArray(checked) ? checked : checked.checked;
      const adUnitKeys = checkedArray.filter((k) =>
        allAdUnitKeys.includes(k as string)
      );
      setCheckedKeys(adUnitKeys);
    },
    []
  );
  ...
  // ---- 行内编辑 oCPM ----
  const startOcpmEditing = (record: AdPlan) => {
    setEditingOcpmKey(record.key);
    setEditOcpmValue(record.ocpm || 0);
  };

  const saveOcpmEditing = () => {
    message.success(`oCPM 已更新为 ¥${editOcpmValue.toFixed(1)}`);
    setEditingOcpmKey('');
  };

  const cancelOcpmEditing = () => {
    setEditingOcpmKey('');
  };
  ...
  const columns: ColumnsType<AdPlan> = [
    ...
    },
    {
      ...
      key: 'ocpm',
      width: 120,
      render: (value: number | undefined, record: AdPlan) => {
        if (editingOcpmKey === record.key) {
          return (
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <InputNumber
                value={editOcpmValue}
                onChange={(val) => setEditOcpmValue(val || 0)}
                size="small"
                style={{ width: 80 }}
                min={0}
                step={0.1}
                precision={1}
                addonBefore="¥"
                onPressEnter={saveOcpmEditing}
              />
              <Button
                type="text"
                size="small"
                icon={<CheckOutlined />}
                onClick={saveOcpmEditing}
                style={{ color: '#52c41a' }}
              />
              <Button
                type="text"
        ...
        return (
          <div
            ...
            onClick={() => startOcpmEditing(record)}
          >
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 12 }}>
          <Input
            placeholder="搜索公司/App/账户/计划组/项目/单元"
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span>已选 {checkedKeys.length} 个单元</span>
      ...
      >
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '0 24px 16px' }}>
          <Table<AdPlan>
            ...
            size="middle"
            rowClassName={(record) =>
              editingKey === record.key || editingOcpmKey === record.key ? 'editing-row' : ''
            }
          />
        </div>
      </Card>
